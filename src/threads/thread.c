...
// mod
struct thread *get_child_proc (int tid);
void remove_child_proc (struct thread *child);
...
void *sbrk(intptr_t increment) {
    ...
    if (t->heap_base == NULL) {
        t->heap_base = (void *)(t->stack_top - PGSIZE);
        t->heap_top = t->heap_base;
    }

    void *new_heap_top = (void *)((uint8_t *)t->heap_top + increment);

    if (new_heap_top >= t->stack_top) {
        return (void *)-1; // Lỗi: Heap vượt qua stack
    }

    if (pg_round_up(new_heap_top) > pg_round_up(t->heap_top)) {
        void *page = pg_round_up(t->heap_top);
        for (; page < pg_round_up(new_heap_top); page += PGSIZE) {
            if (!install_page(page, palloc_get_page(PAL_USER), true)) {
                return (void *)-1;
            }
        }
    }

    void *old_heap_top = t->heap_top;
    t->heap_top = new_heap_top;
    return old_heap_top;
}
